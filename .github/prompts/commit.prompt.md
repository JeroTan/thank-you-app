...
2. **Git Knowledge Rules**: Branching rules in `.github/knowledge/git/git-branching.md` and commit conventions in `.github/knowledge/git/git-commit.md`.
...
2. **Branching Strategy (Enforcing git-branching.md)**
   - Review `.github/knowledge/git/git-branching.md` (if it exists).
...
3. **Draft the Commit (Enforcing git-commit.md)**
   ...
   - Generate a suggested commit message strictly following the Conventional Commits format found in `.github/knowledge/git/git-commit.md` (e.g., `feat(auth): implement JWT validation logic`).
