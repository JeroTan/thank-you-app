---
name: "tangram-design"
description: "Tangram command /tangram:design. Architect or update the solution across 6 pillars, prioritizing User Prompt, Project Knowledge, and Project Studies over AI defaults."
---

Codex adaptation of .codex/workflows/tangram/commands/tangram/preconstruct/design.toml.

Use this skill when the user asks for /tangram:design, $tangram-design, or the corresponding Tangram workflow in natural language. Codex does not load source workflow .toml command files directly; this SKILL.md carries the converted prompt.
...
1. **Step 1: Check State & Knowledge Scan**
   ...
   - **Knowledge Scan**: Scan `tangram/knowledge/**` (if it exists) for project-specific rules and standards, and the corresponding `.agents/knowledge/` subdirectories for framework standards.
   - *Requirement*: You MUST finish reading these files before proceeding to Step 2.

2. **Step 2: Targeted Internet Research**
   - **Once the Knowledge Scan is complete**, use `Codex web search` to find the latest documentation, security advisories, or platform-specific best practices (e.g., "Human Interface Guidelines" for Desktop/Mobile or "CLI UX" for terminal apps) for the technologies identified in Step 1.
...
3. **Step 3: Analyze the Change & Consult**
   If in **Update Mode**, acknowledge the specific modification. If a user instruction contradicts an established House Rule in /knowledge/, point it out and ask: "This change overrides our established standard in /knowledge/. Should I proceed?"
